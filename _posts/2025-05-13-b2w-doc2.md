...
### enable communication between Jetson and PC5
Sometimes Jetson can’t reach PC5 directly because of their wifi ip addresses are not under same router. Lets say PC5 wifi ip address is under `10.206.*`, route via Jetson's Wi-Fi IP:
```bash
# 
sudo ip route add 10.206.0.0/16 via 192.168.123.166 dev eth0
```
